...
export default function LiveMeetingPanel({ onSaveComplete }: { onSaveComplete: () => void }) {
  ...
  return (
    <div className="flex flex-col gap-4 min-h-full">
      <div className="flex flex-col md:flex-row md:items-center justify-between rounded-2xl bg-white p-4 shadow-sm dark:bg-racing-900 border border-gray-100/80 dark:border-racing-800 gap-4">
        <div className="flex-1">
          <div className="flex flex-wrap items-center gap-3 mb-1">
            ...
            {isRecording && (
              <span className="rounded-full bg-red-500/10 px-2.5 py-0.5 font-mono text-sm font-bold tabular-nums text-red-500">
          ...
          {isRecording && (
            <div className="mb-3 flex items-center gap-2">
              <span className="text-[10px] uppercase tracking-wide text-gray-400">{t('live.audioLevel')}</span>
              <div className="flex h-2 flex-1 max-w-xs gap-0.5">
                {Array.from({ length: 12 }).map((_, i) => (
                  <div
                    key={i}
                    className={`flex-1 rounded-sm transition-colors duration-75 ${
                      audioLevel * 12 > i
                        ? audioLevel > 0.6 ? 'bg-red-400' : audioLevel > 0.25 ? 'bg-accent' : 'bg-gray-300 dark:bg-racing-600'
                        : 'bg-gray-100 dark:bg-racing-800'
                    }`}
                  />
                ))}
              </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          ...
          {!isRecording ? (
            <button
              ...
              className="flex items-center gap-2 rounded-xl bg-accent px-4 py-2 text-sm font-bold text-white shadow-md hover:bg-accent-dark transition-all"
            >
      ...
      <div className="flex flex-col gap-4">
        <div className="flex flex-col rounded-2xl bg-white p-4 shadow-sm dark:bg-racing-900 border border-gray-100/80 dark:border-racing-800 min-h-[280px] sm:min-h-[300px]">
                  ...
                  <li key={i} className="flex flex-col gap-1 rounded-lg bg-gray-50 p-2 dark:bg-racing-800">
                    <div className="flex items-start gap-2">
                      ...
                      <div className="flex-1">
                        ...
                        {item.assignee && (
                          <span className="ml-2 rounded bg-accent/20 px-1.5 py-0.5 text-[10px] text-accent font-bold">@{item.assignee}</span>
                        )}
                      </div>
                    </div>
                    {item.dueDate && (
